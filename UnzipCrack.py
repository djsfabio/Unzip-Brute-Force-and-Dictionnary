...
def attaqueBruteForce(str_zipFile, file_list, indexListFichier):
    ...
    while chosetrue:
        ...
        if(incrementationBoucle == 16):
            chosetrue = False
        incrementationBoucle += 1
        stockList = list(map(list, itertools.product(
            string.digits + string.ascii_letters + string.punctuation + ' ', repeat=incrementationBoucle)))
            ...
            with ZipFile(str_zipFile) as zipObj:
                try:
                    ...
                    print("\nLe mot de passe est \"" + motdepasse +
                          "\" pour l'archive \"" + file_list[indexListFichier] + "\"")
                    chosetrue = False
                    break
                except Exception:
                    continue
...
def main():
    ...
    if(typeAttaque == 1):
        ...
        endTime = time()
        print('Temps total pour trouver le mot de passe: %.2f seconds' %
              (endTime - startTime))
    else:
